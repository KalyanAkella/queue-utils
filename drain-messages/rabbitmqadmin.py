...
EXTRA_VERBS = {
    ...
    'drain':   {'mandatory': ['queue', 'exchange'],
                'optional': {}}
}
...
class Management:
  ...
  config""")
    ...
    def invoke_drain(self):
      upload = self.drain_parse(EXTRA_VERBS['drain'])
      (queue, exchange) = (upload['queue'], upload['exchange'])
      print "Draining messages from %s into %s" % (queue, exchange),
      while True:
        (uri, upload) = self.parse_args(["queue=%s" % queue, 'requeue=false', 'count=1'], EXTRA_VERBS['get'])
        messages = json.loads(self.post(uri, json.dumps(upload)))
        if not len(messages): break
        (uri, upload) = self.parse_args(["routing_key=%s" % messages[0]['routing_key'],
                                         "exchange=%s" % exchange,
                                         "payload=%s" % messages[0]['payload'],
                                         "payload_encoding=%s" % messages[0]['payload_encoding']], EXTRA_VERBS['publish'])
        json.loads(self.post(uri, json.dumps(upload)))
        print ".",
      print "DONE!"
    ...
    def drain_parse(self, obj):
        assert_usage(len(self.args) > 1, 'Both the queue to be drained from and the exchange to be publish to, must be specified')
        mandatory =  obj['mandatory']
        upload = {}
        for arg in self.args:
          assert_usage("=" in arg, 'Argument "{0}" not in format name=value'.format(arg))
          (name, value) = arg.split("=", 1)
          assert_usage(name in mandatory, 'Argument "{0}" not recognised'.format(name))
          upload[name] = value
        return upload
